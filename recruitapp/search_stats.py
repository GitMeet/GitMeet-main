...
class SearchBlock:
    def __init__(self, query, block_type):
        ...
        self.jobs = list(itertools.chain(*[[[job]+[i[0], i[1]] for job in filter(None, i[-1]) if str(self.query).lower() in (' ' if not job['job_description'] else str(job['job_description']).lower()) or str(self.query).lower() in map(lambda x:str(x).lower(), filter(None, job['job_tags'])) or (str(self.query).lower() in map(lambda x:str(x).lower(), filter(None, job['bugs'])) if isinstance(job['bugs'], list) else (' ' if not job['bugs'] else str(job['bugs']).lower())) or str(self.query).lower() in (' ' if not job['job_title'] else str(job['job_title']).lower())] for i in tigerSqlite.Sqlite('/home/jamespetullo/gitmeet/projects.db').get_projectname_owner_teamneeded('projects') if i[-1]]))
